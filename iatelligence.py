...
def request_openai(iat):
    """
    Use the OpenAI API to analyze the imported function names and DLL names 
    in a dictionary of IAT entries.
    
    :param iat: A dictionary of IAT entries, where the keys are the imported function 
    names and the values are the DLL names.
    :return: A list of lists containing the DLL names, imported function names, 
    and OpenAI API responses for each IAT entry
    """
    ...
    with tqdm.tqdm(total=len(iat)) as pbar:
        ...
        for imp_name, dll_name in iat.items():
            # Create a prompt for the OpenAI API with the DLL name and imported
            # function name 
            prompt = f"What is the pirpose of this API, is there a MITRE ATT&CK technique \
            associated and why: '{dll_name}: {imp_name}'?"
...
def main():
    """
    Analyze the Import Address Table (IAT) entries in a PE file using the OpenAI API.
    The PE file to analyze must be provided as an argument when running the script.
    """
    ...
    # Open the PE file and extract the Import Address Table (IAT)
    try:
    ...
    csv_data = tabres.get_csv_string()

    with open("output-gpt-3.5-turbo-instruct.csv", "w", encoding="utf-8") as f:
    #with open("output-davinci-002.csv", "w", encoding="utf-8") as f:
        f.write(csv_data)
